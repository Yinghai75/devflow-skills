...
若从 `df-fix` 或 `df-execute` 止损升级进入，必须先读取 `handoff.md` 止损区块和引用证据；若来源于 `df-review-loop`，还要读取 `dependency_scope`、`safe_to_continue_items` 和未处理 finding；新方案基于止损证据设计，不得从头规划。
